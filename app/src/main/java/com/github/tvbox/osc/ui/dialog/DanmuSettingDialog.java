...
public class DanmuSettingDialog extends BaseDialog {
    ...
    private void initColor() {
        ...
        colors.add(false);
        colors.add(true);
        int defaultPos = SP.INSTANCE.getDanmuColor() ? 1 : 0;
        setAdapter(R.id.trv_color, new ButtonAdapter.SelectDialogInterface<Boolean>() {
            @Override
            public void click(Boolean value, int pos) {
                SP.INSTANCE.setDanmuColor(value);
    ...
    private void initSize() {
        ...
        int index = sizes.indexOf(SP.INSTANCE.getDanmuSizesCale()) + 1;
        ...
        sizeAdd.setOnClickListener((v) -> {
            ...
            size.set(size.get() + 1);
            SP.INSTANCE.setDanmuSizesCale(sizes.get(size.get() - 1));
    ...
    private void initLine() {
        ...
        AtomicReference<Integer> line = new AtomicReference<>( SP.INSTANCE.getDanmuMaxLine());
        lineText.setText(line + "行");
        lineAdd.setOnClickListener((v) -> {
            if (line.get() >= 15) return;
            line.set(line.get() + 1);
            SP.INSTANCE.setDanmuMaxLine(line.get());
            lineText.setText(line + "行");
            EventBus.getDefault().post(new RefreshEvent(RefreshEvent.TYPE_SET_DANMU_SETTINGS, false));
        });
    ...
    private void initAlpha() {
        ...
        AtomicReference<Integer> alpha = new AtomicReference<>((int) (Float.parseFloat(df.format(SP.INSTANCE.getDanmuAlpha())) * 100));
        alphaText.setText(alpha + "%");
        alphaAdd.setOnClickListener((v) -> {
            if (alpha.get() >= 100) return;
            alpha.set(alpha.get() + 10);
            SP.INSTANCE.setDanmuAlpha((float) alpha.get() / 100);
            alphaText.setText(alpha + "%");
            EventBus.getDefault().post(new RefreshEvent(RefreshEvent.TYPE_SET_DANMU_SETTINGS, false));
        });
