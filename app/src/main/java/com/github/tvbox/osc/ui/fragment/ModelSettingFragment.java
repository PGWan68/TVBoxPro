...
public class ModelSettingFragment extends BaseLazyFragment {
    ...
    public static ModelSettingFragment newInstance(int type) {
    ...
    protected void init() {
        ...
        if (bundle != null) {
            type = getArguments().getInt(FLAG);
        }

        switch (type) {
            case 1:
                initDataSourceSettingView();
                break;
            case 2:
...
//    }
    ...
    private void initInfoSettingView() {
        findViewById(R.id.tvInfoSetting).setVisibility(View.VISIBLE);
        ((TextView) findViewById(R.id.tvCurrentVersion)).setText("当前版本：" + DefaultConfig.getAppVersionName(getContext()));
        ...
        findViewById(R.id.llUpdate).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (XXPermissions.isGranted(getContext(), DefaultConfig.StoragePermissionGroup())) {
                    final UpdateChecker checker = new UpdateChecker(DefaultConfig.getAppVersionName(getContext()));
                    Toast.makeText(getContext(), "正在检查更新中...", Toast.LENGTH_SHORT).show();
                    checker.checkThenUpgrade(getContext());
                } else {
                    XXPermissions.with(getContext()).permission(DefaultConfig.StoragePermissionGroup()).request(new OnPermissionCallback() {
                        ...
                        public void onGranted(List<String> permissions, boolean all) {
                            if (all) {
                                Toast.makeText(getContext(), "已获得存储权限，正在检查更新中...", Toast.LENGTH_SHORT).show();
                                final UpdateChecker checker = new UpdateChecker(DefaultConfig.getAppVersionName(getContext()));
                                checker.checkThenUpgrade(getContext());
                            }
                        }
    ...
    private void initSysSettingView() {
        findViewById(R.id.tvSysSetting).setVisibility(View.VISIBLE);
    ...
    private void initPlayerSettingView() {
        findViewById(R.id.tvPlayerSetting).setVisibility(View.VISIBLE);
     ...
     * 首页应用设置
     */
    private void initHomeSettingView() {

        findViewById(R.id.tvHomeSetting).setVisibility(View.VISIBLE);
